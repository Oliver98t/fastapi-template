...
class BaseRouter:
    ...
    def init_routes(self, get_privilege):
        ...
        self.router.put(self.singular_item_slug, response_model=self.model, dependencies=[Depends(get_privilege)])(update)

        patch = self._make_update_func(input_model=self.update_model, orm=self.orm)
        self.router.patch(self.singular_item_slug, response_model=self.model, dependencies=[Depends(get_privilege)])(patch)
        ...
        self.router.post("/", response_model=self.model, dependencies=[Depends(get_privilege)])(create)
...
class UserRouter(BaseRouter):
    def __init__(self):
        ...
        self.add_new_route( path=self.singular_item_slug,
                    method='PUT',
    ...
    def _update_user(   self,
        ...
        updated_user = base_schemas.UserInputHashed(
            ...
            hashed_password=update_data.get('hashed_password', current_user.hashed_password)
        )
